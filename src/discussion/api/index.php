...
function getAllTopics(PDO $db): void
{
    ...
    $sortWhitelist = ['subject', 'author', 'created_at'];
    $sort = in_array($_GET['sort'] ?? '', $sortWhitelist, true) ? $_GET['sort'] : 'created_at';
...
function createTopic(PDO $db, array $data): void
{
    ...
    if (
        !isset($data['subject'], $data['message'], $data['author']) ||
...
function updateTopic(PDO $db, array $data): void
{
    ...
    if (!isset($data['id']) || !is_numeric($data['id'])) {
        sendResponse(['success' => false, 'message' => 'Valid topic id is required.'], 400);
    }
    $id = (int)$data['id'];
    ...
    if (array_key_exists('subject', $data)) {
        $setClauses[] = 'subject = ?';
        $values[] = trim((string)$data['subject']);
    }

    if (array_key_exists('message', $data)) {
        $setClauses[] = 'message = ?';
        $values[] = trim((string)$data['message']);
    }
...
function createReply(PDO $db, array $data): void
{
    ...
    if (
        !isset($data['topic_id'], $data['text'], $data['author']) ||
        trim((string)$data['topic_id']) === '' ||
